...
class FeatureExtractor {
	...
	public:
		...
		void drawBresenham(  double x1,  double y1, double x2, double y2 );
		double** init2Darray(int size);
		double** extractFeatureImage(double* pixels, Sketch* transformed, double sigma, double hsize, int gridWidth, int gridHeight );
};
...
void FeatureExtractor::drawBresenham(  double x1,  double y1, double x2, double y2 )
{
  x1 = round(x1);
  x2 = round(x2);
  y1 = round(y1);
...
double* FeatureExtractor::extract()
{
   ...
   // const double angles[] = {0, 45, 90, 135, 180 };
    int idmFeatureSize = gridWidth*gridHeight*5;
	...
	double* pixels = pixelValues(angles, 45, 45, numAngles);
	double** featImage1 = extractFeatureImage(pixels, transformed, sigma, hsize, gridWidth, gridHeight );
	/*
...
double** FeatureExtractor::extractFeatureImage(double* pixels, Sketch* transformed, double sigma, double hsize, int gridWidth, int gridHeight )
{
	double** featim = init2Darray(2*gridWidth);
	return featim; 
}
